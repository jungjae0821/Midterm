...
public class Midterm {
	public static void main(String[] args) {
		...
		while(true) { // (문제2) 무한루프를 실행할 수 있는 조건절을 입력하세요.
			...
			if(sb.toString().equalsIgnoreCase(s)) { // (문제4) exit가 입력됬는지 확인하는 조건절을 완성해 주세요.
			...
			while(true) {
				...
				char c = (char)('a' + i); 
				if(sb.charAt(index) instanceof index) { // (문제5) StringBuffer 속의 문자와 삽입하는 문자가 다른지를 확인하는 조건절을 완성해 주세요.
